...
Since the finite subgroups of $SL_2(\C)$ has a 2-to-1 correspondence with the finite subgroups of $SO(3)$, they can be associated symmetry groups of solids. Below I have included some nice illustrations.
...
\end{align*}
Since we have just multiplied by $\prod_{g \neq 1} p^g$ in the numerator and the denominator it still equals $\frac{s}{p}$. The bottom is obviously fixed by $g$, but why is it not in $\mathfrak{q}$???
{\color{red} How do I know $p^g$ is not in $\mathfrak{p}$ ???? You localize using the complement of the prime ideal right??} Then since the denominator is fixed and the fraction as a whole is fixed this implies that the numerator is fixed as well.

Secondly I want to show that $\End_R(S)_\mathfrak{p} = \End_{R_\mathfrak{q}}(S_\mathfrak{p})$. {\color{red} ???}

From here we just need to wrap everything together. Since $G$ does not contain any pseudoreflections we get from \cref{thm:unramified_pseudoreflections} that the map is an isomorphism when localizing at any height one prime ideal. Then \cref{lem:height_one_iso} gives us that it's an isomorphism {\color{red} need to show depth}.
\fi
\end{proof}
\end{theorem}
